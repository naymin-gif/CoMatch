...
const Textarea = forwardRef<HTMLTextAreaElement, TextareaProps>(
  ({ label, error, className = '', ...props }, ref) => {
    ...
    const textareaBaseStyle =
      'px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 transition-shadow disabled:opacity-50 disabled:bg-gray-50 disabled:cursor-not-allowed w-full resize-y min-h-[100px] text-gray-900';
      ...
      <div className={`flex flex-col gap-1.5 ${className}`}>
        ...
        {label && (
          <label
            ...
            className="text-mini font-medium text-gray-700"
          >
            {label}
          </label>
        )}
        ...
        {error && <span className="text-xs text-comatch-danger">{error}</span>}
      </div>
    );
  }
);
