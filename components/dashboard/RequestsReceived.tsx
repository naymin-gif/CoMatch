      ...
      <div className="space-y-3">
        {inbound.length === 0 ? (
          ...
          inbound.map((app) => (
            <div
              key={app.id}
              className="flex items-center bg-white p-4 rounded-lg border border-blue-100 shadow-sm"
            >
              {/* Applicant Info */}
              <div className="flex-1 flex items-center gap-3">
                ...
                <div>
                  <span className="block font-medium text-gray-800">
                    {app.profiles?.name || 'Unknown User'}
                  </span>
                </div>
              </div>
              ...
              <div className="flex-[1.5] flex items-center justify-between pr-6 border-r border-gray-200">
                ...
                <button
                  onClick={() =>
                    setSelectedMessage(
                      app.intro_message || 'No message provided.'
                    )
                  }
              ...
              <div className="flex-1 flex justify-end gap-6 pl-6">
                {app.status === 'Pending' ? (
                  ...
                  <span
                    className={`text-sm font-bold ${app.status === 'Approved' ? 'text-green-600' : 'text-red-600'}`}
                  >
                    {app.status}
                  </span>
                )}
