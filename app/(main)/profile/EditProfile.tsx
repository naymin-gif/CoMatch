...
export default function EditProfile({
  ...
  const [skills, setSkills] = useState(
    initialSkills && initialSkills.length > 0 ? [...initialSkills, ''] : ['']
  );
  const [roles, setRoles] = useState(
    initialRoles && initialRoles.length > 0 ? [...initialRoles, ''] : ['']
  );

  const [avatars, setAvatars] = useState<string | StaticImageData | undefined>(
    profile_pic_url
  );
  ...
  const [checked, setChecked] = useState(show_email ?? false);
  ...
  const onSubmit = async (
    ...
    try {
      ...
      if (profileFile) {
        dbPayload.profile_pic_url = await uploadImage(
          profileFile,
          'avatars',
          'avatar'
        );
      ...
      if (backgroundFile) {
        dbPayload.bg_pic_url = await uploadImage(
          backgroundFile,
          'backgrounds',
          'background'
        );
      ...
      <Card className="w-full max-w-4xl mx-auto rounded-[var(--radius-card)] shadow-lg border border-border bg-comatch-background text-card-foreground overflow-hidden p-0">
        ...
        <div className="flex flex-row gap-3 justify-center items-center relative h-48 sm:h-64 w-full bg-muted">
          ...
          <div className="z-10 flex flex-row gap-3">
            ...
            >
              <IoMdRemoveCircleOutline className="mr-2 text-comatch-danger" />{' '}
              Remove Background
            </Button>
          </div>
        </div>
          ...
          <FieldSet>
            <FieldGroup className="w-128">
              ...
              <div className="grid grid-cols-2 gap-4">
                <Field>
                  <FieldLabel htmlFor="city" className="font-heading">
                    City
                  </FieldLabel>
                  <Input
                    id="city"
                ...
                <Field>
                  <FieldLabel htmlFor="country" className="font-heading">
                    Country
                  </FieldLabel>
                  <Input
                    id="country"
            ...
            >
              <div className="flex items-center gap-2">
                <Input
                  ...
                  value={r}
                  onChange={(e) =>
                    handleInputChange(roles, index, e.target.value)
                  }
                  className="w-128"
                />
